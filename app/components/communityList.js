...
const CommunityList = ({ data, title }) => {
    const navigation = useNavigation();
    const renderItem = ({ item }) => {
        const isJoinNewCommunity = item.heading === "Join a New community!"; // Assuming the key for "Join a New community!" is always "4"

        return (
            <TouchableOpacity
                style={[
                    styles.listContainer,
                    isJoinNewCommunity && styles.joinNewCommunityItem,
                ]}
                onPress={() => {
